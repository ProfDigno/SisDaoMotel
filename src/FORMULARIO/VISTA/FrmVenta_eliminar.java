...
import FORMULARIO.BO.BO_venta_eliminar;
import FORMULARIO.BO.BO_venta_item_eliminar;
...
//import IMPRESORA_POS.PosImprimir_Venta_eliminar;
 ...
 * @author Digno
 */
public class FrmVenta_eliminar extends javax.swing.JInternalFrame {
...
//    private EvenRender render=new EvenRender();
    ...
    private venta_eliminar ENTveni = new venta_eliminar();
    private DAO_venta_eliminar DAOveni = new DAO_venta_eliminar();
    private BO_venta_eliminar BOveni = new BO_venta_eliminar();
    private venta_item_eliminar ENTvenii = new venta_item_eliminar();
    private DAO_venta_item_eliminar DAOvenii = new DAO_venta_item_eliminar();
    private BO_venta_item_eliminar BOvenii = new BO_venta_item_eliminar();
...
//    private PosImprimir_Venta_eliminar posveni = new PosImprimir_Venta_eliminar();
...
//    private EvenEstado eveest = new EvenEstado();
    ...
    private String nombreTabla_pri = "venta_eliminar PRINCIPAL";
    private String nombreTabla_sec = "FILTRO";
    private String creado_por = "digno";
    private int idproducto;
    private int fk_idventa_eliminar;
...
//    private double monto_iva10;
    ...
    private void reestableser_venta_eliminar() {
        cargar_usuario_acceso();
        actualizar_tabla_venta_eliminar();
        fk_idventa_eliminar = (eveconn.getInt_ultimoID_mas_uno(conn, ENTveni.getTb_venta_eliminar(), ENTveni.getId_idventa_eliminar()));
        txtidcompra.setText(String.valueOf(fk_idventa_eliminar));
...
//                DAOp.update_producto_precio_compra(conn, ENTp);
            ...
            reestableser_item_venta_eliminar();
            suma_item_producto();
        }
    }

    private void suma_item_producto() {
        monto_eliminar = eveJtab.getDouble_sumar_tabla(tblitem_producto, 8);
        jFtotal_consumo.setValue(monto_eliminar);
        String Smonto_total = String.valueOf(monto_eliminar);
        monto_letra = nroletra.Convertir(Smonto_total, true);
        txtmonto_letra.setText(monto_letra);
    }

    private void reestableser_item_venta_eliminar() {
    ...
    private boolean validar_carga_venta_eliminar() {
...
//        }
    ...
    private void cargar_dato_venta_eliminar() {
        ...
        ENTveni.setC8monto(monto_eliminar);
...
//    private void cargar_dato_caja_detalle_venta_eliminar() {
//        creado_por = ENTusu.getGlobal_nombre();
//        fk_idusuario = ENTusu.getGlobal_idusuario();
//        String descripcion = fk_idventa_eliminar + "-(" + eveest.getCaja_VEN_INTERNA() + ")-"
...
//        ENTccd.setC26monto_interno(monto_eliminar);
//        ENTccd.setC27fk_idventa_interno(fk_idventa_eliminar);
//        ENTccd.setC28monto_garantia(0);
//        ENTccd.setC29fk_idgarantia(0);
//    }

    private void boton_guardar_venta_eliminar() {
        if (validar_carga_venta_eliminar()) {
            cargar_dato_venta_eliminar();
//            cargar_dato_caja_detalle_venta_eliminar();
            BOveni.insertar_venta_eliminar(ENTveni, tblitem_producto);
...
//            poscom.boton_imprimir_pos_compra(conn, fk_idventa_interno);
    ...
    private void boton_anular_venta_eliminar() {
        if (eveJtab.getBoolean_validar_select_mensaje(tblventa_interna, "DEBE SELECCIONAR UNA venta_eliminar PARA ANULAR")) {
            if (evemen.getBooMensaje_warning("ESTAS SEGURO DE ANULAR ESTA venta_eliminar\n"
                    + "ESTO REPONE EL STOCK",
                    "ANULAR venta_eliminar", "ANULAR", "CANCELAR")) {
                int idventa_eliminar = eveJtab.getInt_select_id(tblventa_interna);
//                int idcaja_cierre_detalle = DAOccd.getInt_idcaja_cierre_detalle_por_otro_id(conn, "fk_idventa_eliminar", idventa_eliminar);
//                DAOccd.cargar_caja_cierre_detalle(conn, ENTccd, idcaja_cierre_detalle);
                DAOveni.cargar_venta_eliminar(conn, ENTveni, idventa_eliminar);
                ENTveni.setC1idventa_eliminar(idventa_eliminar);
                ENTveni.setC5estado(eveest.getEst_Anulado());
                ENTvenii.setC10fk_idventa_eliminar(idventa_eliminar);
...
//                ENTccd.setC26monto_interno(0);
                BOveni.update_venta_eliminar_anular(ENTveni,ENTvenii);
                actualizar_tabla_venta_eliminar();
            }
        }
    }
    ...
    private void seleccionar_venta_eliminar() {
        if (tblventa_interna.getSelectedRow() >= 0) {
            int idventa_interna = eveJtab.getInt_select_id(tblventa_interna);
            DAOvenii.actualizar_tabla_venta_item_eliminar_creado(conn, tblitem_venta_interna, idventa_interna);
    ...
    private String getfiltro_venta_eliminar() {
        String filtro_suma = "";
        String filtro_usu = "";
        String filtro_fec = "";
        String filtro_est = "";
    ...
    private void actualizar_tabla_venta_eliminar() {
        DAOveni.actualizar_tabla_venta_eliminar(conn, tblventa_interna, getfiltro_venta_eliminar());
        everen.rendertabla_estados_venta_interno(tblventa_interna, 4);
        suma_venta_eliminar_filtro(getfiltro_venta_eliminar());
    }

    private void suma_venta_eliminar_filtro(String filtro) {
        String titulo = "suma_venta_eliminar_filtro";
        String sql = "select \n"
                + "(sum(vi.monto)) as total \n"
    ...
    private void boton_imprimir_venta_eliminar() {
        if (eveJtab.getBoolean_validar_select_mensaje(tblventa_interna, "SELECCIONE LA TABLA venta_eliminar")) {
            int idventa_eliminar = eveJtab.getInt_select_id(tblventa_interna);
//            posveni.boton_imprimir_pos_venta_interno(conn, idventa_interno);
        }
    }

    public FrmVenta_eliminar() {
        initComponents();
        abrir_formulario();
    }
    ...
    private void initComponents() {
        ...
        jTextField1.setText("VENTA ELIMINAR");
        ...
        panel_crear_ventaLayout.setHorizontalGroup(
            ...
            .addGroup(panel_crear_ventaLayout.createSequentialGroup()
                ...
                .addGroup(panel_crear_ventaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addGroup(panel_crear_ventaLayout.createSequentialGroup()
                        .addComponent(txtidcompra, javax.swing.GroupLayout.PREFERRED_SIZE, 100, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(txtproveedor_nombre)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(txtproveedor_ruc, javax.swing.GroupLayout.PREFERRED_SIZE, 135, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(btnbuscar_persona, javax.swing.GroupLayout.PREFERRED_SIZE, 29, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void formInternalFrameOpened(javax.swing.event.InternalFrameEvent evt) {//GEN-FIRST:event_formInternalFrameOpened
        ...
        ancho_tabla_producto();
        ancho_item_producto();
        DAOveni.ancho_tabla_venta_eliminar(tblventa_interna);
