...
public class DAO_rh_turno {
	...
	private String sql_select = "SELECT idrh_turno,fecha_creado,creado_por,turno,hora_inicio,hora_salida,tolera_llega_tarde,tolera_sale_antes,domindo,lunes,martes,miercoles,jueves,viernes,sabado,activo,incluye_dos_dia FROM rh_turno order by 1 desc;";
	...
	public void insertar_rh_turno(Connection conn, rh_turno rhtu){
		...
		try {
			...
			pst.setTime(5,evefec.getTime_sistema());
			pst.setTime(6,evefec.getTime_sistema());
	...
	public void update_rh_turno(Connection conn, rh_turno rhtu){
		...
		try {
			...
			pst.setTime(4,evefec.getTime_sistema());
			pst.setTime(5,evefec.getTime_sistema());
