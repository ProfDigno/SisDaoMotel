...
public class DAO_rh_liquidacion_detalle {
    ...
    private String sql_anu_vale = "UPDATE rh_liquidacion_detalle SET monto_vale=?,estado=? WHERE fk_idrh_vale=?;";
    private String sql_anu_descu = "UPDATE rh_liquidacion_detalle SET monto_descuento=?,estado=? WHERE fk_idrh_descuento=?;";
    ...
    public void update_rh_liquidacion_detalle_anular_vale(Connection conn, rh_liquidacion_detalle rld) {
        String titulo = "update_rh_liquidacion_detalle_anular_vale";
        PreparedStatement pst = null;
        try {
            pst = conn.prepareStatement(sql_anu_vale);
            pst.setDouble(1, rld.getC6monto_vale());
            pst.setString(2, rld.getC8estado());
            pst.setInt(3, rld.getC11fk_idrh_vale());
            pst.execute();
            pst.close();
    ...
    public void update_rh_liquidacion_detalle_anular_descuento(Connection conn, rh_liquidacion_detalle rld) {
        String titulo = "update_rh_liquidacion_detalle_anular_descuento";
        PreparedStatement pst = null;
        try {
            pst = conn.prepareStatement(sql_anu_descu);
            pst.setDouble(1, rld.getC5monto_descuento());
            pst.setString(2, rld.getC8estado());
            pst.setInt(3, rld.getC10fk_idrh_descuento());
            pst.execute();
            pst.close();
            evemen.Imprimir_serial_sql(sql_anu_descu + "\n" + rld.toString(), titulo);
            evemen.modificado_correcto(mensaje_update, true);
        } catch (Exception e) {
            evemen.mensaje_error(e, sql_anu_descu + "\n" + rld.toString(), titulo);
        }
    }
}
