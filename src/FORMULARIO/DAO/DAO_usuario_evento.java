...
public class DAO_usuario_evento {
    ...
    private String sql_insert = "INSERT INTO usuario_evento(idusuario_evento,fecha_creado,creado_por,codigo,nombre,descripcion,mensaje_error,fk_idusuario_tipo_evento,fk_idusuario_formulario) VALUES (?,?,?,?,?,?,?,?,?);";
    private String sql_update = "UPDATE usuario_evento SET fecha_creado=?,creado_por=?,codigo=?,nombre=?,descripcion=?,mensaje_error=?,fk_idusuario_tipo_evento=?,fk_idusuario_formulario=? WHERE idusuario_evento=?;";
    private String sql_select = "SELECT idusuario_evento as idue,codigo,nombre,mensaje_error FROM usuario_evento order by 1 desc;";
    private String sql_cargar = "SELECT idusuario_evento,fecha_creado,creado_por,codigo,nombre,descripcion,mensaje_error,fk_idusuario_tipo_evento,fk_idusuario_formulario FROM usuario_evento WHERE idusuario_evento=";
    ...
    public void insertar_usuario_evento(Connection conn, usuario_evento usev) {
        ...
        try {
            ...
            pst.setString(7, usev.getC7mensaje_error());
            pst.setInt(8, usev.getC8fk_idusuario_tipo_evento());
            pst.setInt(9, usev.getC9fk_idusuario_formulario());
            pst.execute();
            pst.close();
    ...
    public void update_usuario_evento(Connection conn, usuario_evento usev) {
        ...
        try {
            ...
            pst.setString(6, usev.getC7mensaje_error());
            pst.setInt(7, usev.getC8fk_idusuario_tipo_evento());
            pst.setInt(8, usev.getC9fk_idusuario_formulario());
            pst.setInt(9, usev.getC1idusuario_evento());
            pst.execute();
            pst.close();
    ...
    public void cargar_usuario_evento(Connection conn, usuario_evento usev, int idusuario_evento) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                usev.setC7mensaje_error(rs.getString(7));
                usev.setC8fk_idusuario_tipo_evento(rs.getInt(8));
                usev.setC9fk_idusuario_formulario(rs.getInt(9));
