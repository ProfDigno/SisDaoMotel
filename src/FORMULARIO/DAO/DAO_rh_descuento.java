...
public class DAO_rh_descuento {
    ...
    private String sql_anular = "UPDATE rh_descuento SET monto_descuento=?,estado=?,monto_letra=? WHERE idrh_descuento=?;";
    ...
    public void update_rh_descuento_anular(Connection conn, rh_descuento rhde) {
        String titulo = "update_rh_descuento_anular";
        PreparedStatement pst = null;
        try {
            pst = conn.prepareStatement(sql_anular);
            pst.setDouble(1, rhde.getC5monto_descuento());
            pst.setString(2, rhde.getC6estado());
            pst.setString(3, rhde.getC8monto_letra());
            pst.setInt(4, rhde.getC1idrh_descuento());
            pst.execute();
            pst.close();
