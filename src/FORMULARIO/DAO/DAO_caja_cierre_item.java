...
public class DAO_caja_cierre_item {
    ...
    public void insertar_tabla_caja_cierre_item_CERRAR(Connection conn, String creado_por,int fk_idusuario) {
        String titulo = "insertar_tabla_caja_cierre_item_CERRAR";
        String sql = ""
//                + "DROP SEQUENCE IF EXISTS id_seq;\n"
//                + "CREATE SEQUENCE id_seq ;"
                + "INSERT INTO caja_cierre_item(\n"
                + "            idcaja_cierre_item, fecha_creado, creado_por, fk_idcaja_cierre, \n"
                + "            fk_idcaja_cierre_detalle)\n"
                + "select nextval('id_seq')  as idcaja_cierre_item,\n"
                + "current_timestamp as fecha_creado,('"+creado_por+"') as creado_por,\n"
                + "coalesce ((select max(cc.idcaja_cierre) from caja_cierre cc),1)  as fk_idcaja_cierre,cd.idcaja_cierre_detalle\n"
                + "from caja_cierre_detalle cd\n"
                + "where cd.es_cerrado=false \n"
                //+ "and cd.estado='EMITIDO'\n"
                + "and cd.fk_idusuario="+fk_idusuario
                + " order by cd.idcaja_cierre_detalle asc;";
