...
public class PosImprimir_Venta {
    ...
    private void cargar_datos_venta(Connection conn, int idventa) {
        ...
        String sql = "select v.idventa as idv,\n"
                ...
                + "case when TRIM(hr.estado)='OCUPADO' "
                + "     then TRIM(to_char(current_timestamp,'yyyy-MM-dd HH24:MI')) "
                + "     else TRIM(to_char(hr.fec_ocupado_fin,'yyyy-MM-dd HH24:MI')) end as fec_fin,\n"
                + "hr.nro_habitacion as habitacion,\n"
                + "case when TRIM(hr.estado)='OCUPADO' "
                + "     then TRIM(to_char((current_timestamp-hr.fec_ocupado_inicio), 'HH24:MI:ss'))"
                + "     else TRIM(to_char((hr.fec_ocupado_fin-hr.fec_ocupado_inicio), 'HH24:MI:ss')) end as tiempo,\n"
                + "case when hr.es_por_hora=true then ('POR HORA-'||hr.estado) \n"
                + "     when hr.es_por_dormir=true then 'DORMIR'\n"
                + "     when hr.es_hospedaje=true then 'HOSPEDAJE'\n"
