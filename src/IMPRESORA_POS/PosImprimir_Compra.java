...
//    private static json_imprimir_pos jsprint = new json_imprimir_pos();
    ...
    private void cargar_datos_compra(Connection conn, int idcompra_insumo) {
        ...
        String sql = "select ci.idcompra as idcompra,\n"
                + "to_char(ci.fecha_creado,'yyyy-MM-dd HH24:MI') as fecha, \n"
                + "ci.observacion,u.nombre as usuario,\n"
                + "TRIM(to_char(ci.monto_total,'999G999G999')) as monto,\n"
                ...
                + "TRIM(to_char((ici.cantidad*ici.precio_compra),'999G999G999'))  as subtotal,"
                + "p.nombre as proveedor \n"
                + "from compra ci,usuario u,compra_item ici,persona p \n"
                + "where ci.fk_idusuario=u.idusuario \n"
                + "and ci.fk_idpersona=p.idpersona \n"
